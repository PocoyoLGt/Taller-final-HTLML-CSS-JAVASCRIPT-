 const main = () =>{
     let element = document.querySelector("#cuerpo");
     console.log(element);
     let element2 = document.getElementById("subtitulo2");
     console.log(element2);
     let element3 = document.getElementById("subtitulo");
     console.log(element3);
     let btnEjecutar = document.querySelector("#btnEjecutar");
     btnEjecutar.addEventListener("click", ejecutar);
   
 }
 const cambiarColor = (id) => {
    let element = document.querySelector(`#${id}`);
    element.classList.add("estilo2");
}
const cambiarColor2 = (id) => {
    let element = document.querySelector(`#${id}`);
    element.classList.add("fondosub");
}

const removerEstilo = (id, estilo) => {
    let element = document.querySelector(`#${id}`);
    element.classList.remove(estilo);
}

const ejecutar = () => {
    let element = document.querySelector(`#slcOpciones`);
    let parrafo = document.querySelector(`#cuerpo`);
    switch (Number(element.value)) {
        case 1:
            parrafo.classList.add("estilo2");
            break;
        case 2:
            parrafo.classList.add("fondosub");
            break;
        case 3:
            parrafo.classList.remove("estilo2","fondosub");
            break; 
        default:
            console.log(element.value);
    }
}
var click = 1;
function myFunction() {
    
    if(click == 1){
      document.getElementById("prueba").style.display = 'none';
   
    click= click+1;  
    }else{
         document.getElementById("prueba").style.display = 'block';
        click =1;
    }
    
    
  }
  function myFunction2() {
    
    if(click == 1){
      document.getElementById("prueba2").style.display = 'none';
   
    click= click+1;  
    }else{
         document.getElementById("prueba2").style.display = 'block';
        click =1;
    }
    
    
  }
  function myFunction3() {
    
    if(click == 1){
      document.getElementById("tercero2").style.display = 'none';
   
    click= click+1;  
    }else{
         document.getElementById("tercero2").style.display = 'block';
        click =1;
    }
    
    
  }
  function myFunction4() {
    
    if(click == 1){
      document.getElementById("cuarto2").style.display = 'none';
   
    click= click+1;  
    }else{
         document.getElementById("cuarto2").style.display = 'block';
        click =1;
    }
    
    
  }
  function myFunction5() {
    
    if(click == 1){
      document.getElementById("quinto2").style.display = 'none';
   
    click= click+1;  
    }else{
         document.getElementById("quinto2").style.display = 'block';
        click =1;
    }
    
    
  }



